...
with st.sidebar:
    ...
    with st.expander("➕ Add Webmail Account", expanded=len(st.session_state.webmail_accounts) == 0):
        with st.form("add_account"):
            ...
            wm_url = st.text_input("Webmail URL", 
                value="https://webmail.yourdomain.com",
                help="Usually: https://webmail.yourdomain.com or https://yourdomain.com:2096")
            
            col1, col2 = st.columns(2)
            with col1:
                wm_daily = st.number_input("Daily Limit", value=100, min_value=10, max_value=500)
            with col2:
                wm_hourly = st.number_input("Hourly Limit", value=15, min_value=5, max_value=50)
...
st.caption("Scrape leads from Google Maps & Yellow Pages → Send via FREE webmail")
...
with tab1:
    ...
    col1, col2 = st.columns(2)
    with col1:
        query = st.text_input("Business Category", value="Plumbers", 
    ...
    if st.session_state.scanning:
        ...
        if st.button("🚀 Start Scraping", type="primary"):
            ...
            cmd = [sys.executable, "run_scraper_cli.py", "--query", query, "--location", location]
            if headful:
                cmd.append("--headful")
    ...
    if st.session_state.scanning and st.session_state.process:
        ...
        if retcode is not None:
            ...
            if stderr.strip():
                ...
                st.success(f"✓ Scraping complete! {len(st.session_state.leads_df)} total leads in database.")
    ...
    if not st.session_state.webmail_accounts:
        ...
        with col1:
            ...
            if 'address' in leads_with_email.columns:
                locations = sorted([str(x) for x in leads_with_email['address'].dropna().unique() if str(x).strip()])
                loc_filter = st.multiselect("Filter by Location", locations)
                if loc_filter:
                    leads_with_email = leads_with_email[leads_with_email['address'].isin(loc_filter)]
            ...
            if 'category' in leads_with_email.columns:
                ...
                if cat_filter:
                    leads_with_email = leads_with_email[leads_with_email['category'].apply(lambda x: any(c in str(x) for c in cat_filter))]
            ...
            # Limit selection
            max_to_send = st.slider("Max emails to send", 
                min_value=1, 
        ...
        with col2:
            ...
            st.caption("💡 Available variables: {{ company }}, {{ first_name }}, {{ location }}, {{ sender_name }}")
            ...
            if not leads_filtered.empty:
                ...
                test_subj = Template(subject).render(
                    ...
                    first_name=test_lead['name'].split()[0] if ' ' in test_lead['name'] else test_lead['name']
                )
        ...
        session_name = st.text_input("Session / Campaign Name", value=session_default, help="Identify this campaign session to track follow-ups")
        ...
        with col3:
            st.info(f"Sending ~{max_to_send} emails with {delay_min}-{delay_max}s delays")
            ...
            with st.spinner("Sending emails via webmail..."):
                ...
                async def run_campaign():
                    ...
                    for _, row in leads_filtered.head(max_to_send).iterrows():
                        first_name = row['name'].split()[0] if ' ' in str(row['name']) else str(row['name'])
                        recipients.append({
                            'email': row['email'],
                            'company': row['name'],
                            'first_name': first_name,
                            'location': location,
                            'sender_name': st.session_state.webmail_accounts[0].name,
                            'sender_company': 'Digital Marketing Agency',
                            'sender_email': st.session_state.webmail_accounts[0].email
                        })
                ...
                try:
                    ...
                    for i, (_, row) in enumerate(leads_filtered.head(max_to_send).iterrows()):
                        df.loc[df.index == row.name, 'campaign_status'] = 'Sent'
                        df.loc[df.index == row.name, 'last_contact'] = datetime.now().isoformat()
                        ...
                        df.loc[df.index == row.name, 'contact_count'] = 1 if pd.isna(count) else count + 1
                    save_leads(df)
    ...
    if df.empty:
        ...
        with chart_col2:
            if 'category' in df.columns:
                ...
                cat_counts = cats[cats != ''].value_counts().head(5)
        ...
        analytics_tabs = st.tabs(["📋 All Leads", "✉️ Contacted Leads (Sessions)", "📅 Follow-Ups"])
        
        with analytics_tabs[0]:
            col1, col2 = st.columns([1, 4])
            with col1:
                show_missing = st.checkbox("Only missing email", value=False, key="chk_missing")
            
            display_df = df.copy()
            if show_missing and 'email' in df.columns:
                display_df = display_df[display_df['email'].isna() | (display_df['email'] == '')]
        ...
        with analytics_tabs[1]:
            ...
            if 'session_id' in df.columns and not df[df['session_id'].notna()].empty:
                ...
                sessions = contacted_df[['session_id', 'session_name', 'last_contact']].drop_duplicates(subset=['session_id']).sort_values('last_contact', ascending=False)
                
                selected_session_name = st.selectbox("Select Campaign Session", ["All Sessions"] + sessions['session_name'].tolist())
                
                if selected_session_name != "All Sessions":
                    session_id = sessions[sessions['session_name'] == selected_session_name].iloc[0]['session_id']
                    contacted_df = contacted_df[contacted_df['session_id'] == session_id]
                ...
                st.dataframe(
                    ...
                    column_config={
                        'website': st.column_config.LinkColumn("Website"),
                        'email': st.column_config.LinkColumn("Email"),
                        'campaign_status': st.column_config.TextColumn("Status", default="Sent 🟢")
                    }
                )
            else:
        ...
        with analytics_tabs[2]:
            ...
            if 'last_contact' in df.columns and 'campaign_status' in df.columns:
                ...
                if not followup_df.empty:
                    now = datetime.now()
                    followup_df['days_since_contact'] = followup_df['last_contact'].apply(
                        lambda x: (now - datetime.fromisoformat(x)).days if pd.notna(x) else 0
                    )
                    followup_df = followup_df[followup_df['days_since_contact'] >= 3].sort_values('days_since_contact', ascending=False)
                    ...
                    st.dataframe(
                        ...
                        column_config={
                            'website': st.column_config.LinkColumn("Website"),
                            'email': st.column_config.LinkColumn("Email"),
                            'days_since_contact': st.column_config.NumberColumn("Days Since Contact", format="%d days")
                        }
                    )
                else:
